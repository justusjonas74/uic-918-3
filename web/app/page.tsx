...
export default function Home() {
  ...
  const renderUFlexDetails = (containerData: any) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {travelers.length > 0 && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800, marginBottom: '0.75rem', textTransform: 'uppercase' }}>
              <User size={18} style={{ color: 'var(--accent-pink)' }} />
              Reisende
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
              {travelers.map((traveler: any, idx: number) => (
                <div key={idx} className="brutal-card" style={{ padding: '0.75rem 1rem', margin: 0, boxShadow: '2px 2px 0px #000', borderWidth: '2px', flex: '1 1 200px' }}>
                  <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>
                    {traveler.firstName} {traveler.lastName}
                  </div>
                  <div style={{ fontSize: '0.85rem', marginTop: '0.25rem', color: '#555' }}>
                    Typ: <span className="brutal-badge brutal-badge-teal" style={{ borderWidth: '1px', padding: '0.1rem 0.4rem' }}>
                      {typeof traveler.passengerType === 'object' ? Object.keys(traveler.passengerType)[0] : (traveler.passengerType || 'Erwachsener')}
                    </span>
                  </div>
                  {(traveler.yearOfBirth || traveler.monthOfBirth || traveler.dayOfBirthInMonth) && (
                    <div style={{ fontSize: '0.85rem', marginTop: '0.25rem' }}>
                      Geburtsdatum: {traveler.dayOfBirthInMonth || '?'}.{traveler.monthOfBirth || '?'}.{traveler.yearOfBirth || '?'}
                    </div>
                  )}
                </div>
              ))}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #000' }}>
            <tbody>
              <tr style={{ borderBottom: '2px solid #000' }}>
                <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', width: '40%', borderRight: '2px solid #000' }}>Herausgeber</td>
                <td style={{ padding: '0.5rem' }}>{issuing.issuerName || issuing.issuerNum || 'Unbekannt'} ({issuing.issuerNum || issuing.securityProviderNum})</td>
              </tr>
              {(issuing.issuingYear || issuing.issuingDay) && (
                <tr style={{ borderBottom: '2px solid #000' }}>
                  <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', borderRight: '2px solid #000' }}>Datum der Ausstellung</td>
                  <td style={{ padding: '0.5rem' }}>Jahr: {issuing.issuingYear} | Tag im Jahr: {issuing.issuingDay}</td>
                </tr>
              )}
              {issuing.issuingTime !== undefined && (
                <tr style={{ borderBottom: '2px solid #000' }}>
                  <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', borderRight: '2px solid #000' }}>Uhrzeit der Ausstellung</td>
                  <td style={{ padding: '0.5rem' }}>{String(Math.floor(issuing.issuingTime / 60)).padStart(2, '0')}:{String(issuing.issuingTime % 60).padStart(2, '0')} Uhr</td>
                </tr>
              )}
              {issuing.issuerPNR && (
                <tr style={{ borderBottom: '2px solid #000' }}>
                  <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', borderRight: '2px solid #000' }}>PNR (Auftragsnummer)</td>
                  <td style={{ padding: '0.5rem', fontFamily: 'monospace', fontWeight: 'bold' }}>{issuing.issuerPNR}</td>
                </tr>
              )}
              {issuing.currency && (
                <tr>
                  <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', borderRight: '2px solid #000' }}>Währung</td>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {documents.map((doc: any, idx: number) => {
                ...
                return (
                  <div key={idx} className="brutal-card" style={{ padding: '1rem', margin: 0, borderWidth: '2px', boxShadow: '2px 2px 0px #000' }}>
                    ...
                    {ticketDetails && typeof ticketDetails === 'object' ? (
                        ...
                        {Object.entries(ticketDetails).map(([key, val]: [string, any]) => {
                          if (typeof val === 'object') {
                            ...
                            if (key === 'tariffs' && val.TariffType) {
                              return (
                                <div key={key} style={{ display: 'flex', borderBottom: '1px solid #eee', padding: '0.25rem 0' }}>
                      ...
                      <div style={{ fontSize: '0.85rem', color: '#555' }}>
                        Keine detaillierten Felder vorhanden.
                      </div>
                    )}
                  </div>
                );
          ...
          {Object.entries(data).map(([key, val]: [string, any]) => {
            ...
            return (
              <tr key={key} style={{ borderBottom: '2px solid #000' }}>
                <td style={{ padding: '0.5rem', fontWeight: 'bold', background: '#f5f5f5', width: '35%', borderRight: '2px solid #000', fontSize: '0.85rem' }}>
                  {key}
                </td>
  ...
  return (
    <div>
      {/* Title Header */}
      <header className="brutal-card" style={{ background: 'var(--accent-yellow)', margin: '2rem 0', padding: '1.5rem', textAlign: 'center' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '-0.05em', lineHeight: 1 }}>
          UIC 918-3 Barcode Decoder
        </h1>
        <p style={{ marginTop: '0.5rem', fontSize: '1.1rem', fontWeight: 700 }}>
          [ NEO-BRUTALISTISCHES INTERFACE ZUM ENTSCHLÜSSELN VON BAHN-TICKETS ]
        </p>
      </header>
      ...
      <div className="brutal-grid">
        ...
        <div>
          ...
          <div className="brutal-tab-container">
            <button
              ...
              onClick={() => { setActiveTab('hex'); setError(null); }}
            >
              ...
              {activeTab === 'hex' ? (
                <div>
                  ...
                  <p style={{ fontSize: '0.8rem', marginTop: '0.5rem', color: '#555' }}>
                    Tipp: Der Hex-String muss mit dem Ticket-Header beginnen (z.B. `#UT02`, welches in Hex `2355543032` ist).
                  </p>
                </div>
              ) : (
                <div>
                  ...
                  {filePreview && (
                    <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
                      <div style={{ fontWeight: 800, marginBottom: '0.5rem', fontSize: '0.9rem', textTransform: 'uppercase' }}>
                        Ausgewähltes Bild:
                      </div>
                      <div style={{ border: '2px solid #000', display: 'inline-block', padding: '0.5rem', background: '#fff' }}>
                        <img
                          src={filePreview}
        ...
        <div>
          ...
          {!loading && !error && !result && (
            <div className="brutal-card" style={{ minHeight: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', borderStyle: 'dashed', background: 'transparent', boxShadow: 'none' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: '#777', textAlign: 'center', marginTop: '0.25rem', maxWidth: '300px' }}>
                Füge links einen Hex-String ein oder lade ein Barcode-Bild hoch. Die dekodierten Ticket-Daten erscheinen hier.
              </p>
            </div>
          )}
          ...
          {/* Loading State */}
          {loading && (
            <div className="brutal-card" style={{ minHeight: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: '#fff' }}>
              <div style={{ animation: 'spin 2s linear infinite', border: '8px solid #f3f3f3', borderTop: '8px solid #000', borderRadius: '50%', width: '50px', height: '50px', marginBottom: '1rem' }} />
              <p style={{ fontWeight: 900, textTransform: 'uppercase', fontSize: '1.25rem' }}>
                Dekodiere Daten...
              </p>
          ...
          {error && (
            <div className="brutal-card" style={{ background: 'var(--accent-pink)', color: '#fff', padding: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 900, fontSize: '1.25rem', marginBottom: '1rem', textTransform: 'uppercase' }}>
                <XCircle size={24} style={{ color: '#fff' }} />
                Dekodierung Fehlgeschlagen
              </div>
              <p style={{ fontWeight: 800, fontSize: '1rem', background: '#000', padding: '0.75rem', border: '2px solid #fff' }}>
                {error}
              </p>
          ...
          {result && (
            <div>
                ...
                <div className="signature-status status-unknown">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', flexDirection: 'column' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'none' }}>
                      (Für RICS {result.header.rics} und Key-ID {result.header.key_id} wurde kein lokaler Zertifikatsschlüssel gefunden.)
                    </span>
                  </div>
                </div>
              ) : (
              ...
              <div className="brutal-card" style={{ background: 'var(--bg-white)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                  <div style={{ border: '2px solid #000', padding: '0.5rem', background: '#f9f9f9' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 900, color: '#666', textTransform: 'uppercase' }}>Bahnunternehmen (RICS)</div>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem' }}>{formatRICS(result.header.rics)}</div>
                  </div>
                  <div style={{ border: '2px solid #000', padding: '0.5rem', background: '#f9f9f9' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 900, color: '#666', textTransform: 'uppercase' }}>Schnittstellen-ID (Key ID)</div>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem', fontFamily: 'monospace' }}>{result.header.key_id}</div>
                  </div>
                  <div style={{ border: '2px solid #000', padding: '0.5rem', background: '#f9f9f9' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 900, color: '#666', textTransform: 'uppercase' }}>Barcode Version</div>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem' }}>Version {result.version}</div>
                  </div>
                  <div style={{ border: '2px solid #000', padding: '0.5rem', background: '#f9f9f9' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 900, color: '#666', textTransform: 'uppercase' }}>UMID (Unique Ticket ID)</div>
                    <div style={{ fontWeight: 800, fontSize: '0.95rem', fontFamily: 'monospace', wordBreak: 'break-all' }}>{result.header.umid}</div>
                  </div>
                </div>
              ...
              <h2 style={{ fontSize: '1.5rem', fontWeight: 900, textTransform: 'uppercase', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <FileCode size={24} />
                Daten-Container
              </h2>
                ...
                return (
                  <div key={container.id} className="brutal-card" style={{ padding: '0 0 1rem 0', overflow: 'hidden' }}>
                    ...
                    >
                      ...
                      <div style={{ transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.15s ease', fontWeight: 900 }}>
                        <ArrowRight size={20} />
                      </div>
                    </div>
                      ...
                      <div style={{ padding: '1rem' }}>
                        ...
                        {viewMode === 'details' ? (
                          ...
                          <div>
                            ...
                            <pre className="brutal-code">
                              {JSON.stringify(container.container_data, null, 2)}
                            </pre>
                          </div>
                        )}
                      </div>
      ...
      {/* Footer */}
      <footer style={{ marginTop: '4rem', borderTop: '4px solid #000', padding: '2rem 0', textAlign: 'center', fontSize: '0.8rem' }}>
